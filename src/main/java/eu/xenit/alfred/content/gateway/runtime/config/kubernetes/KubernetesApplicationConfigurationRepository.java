...
public class KubernetesApplicationConfigurationRepository implements ApplicationConfigurationRepository {
    ...
    @Override
    public Mono<ApplicationConfiguration> getApplicationConfiguration(String appId) {
                ...
                .watch(new Watcher<>() {
                    ...
                    public void eventReceived(Action action, Secret resource) {
                        secretMapper.apply(resource).ifPresent(fragment -> {
                            var appConfig = configs.computeIfAbsent(fragment.getApplicationId(),
                                    ComposedApplicationConfiguration::new);
                            ...
                            switch (action) {
                                case ADDED, MODIFIED -> appConfig.putFragment(fragment);
                                case DELETED -> appConfig.removeFragment(fragment.getConfigurationId());
